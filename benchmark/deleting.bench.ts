...
for (const samples of [1_000, 5_000, 10_000, 50_000, 100_000]) {
  Deno.bench({
    name: `Delete ${samples} triples (NodeID format, high duplicates)`,
    group: "TribbleDB Deletion (NodeID)",
    fn: (bench) => {
      const ID_LENGTH = 5;
...
for (const samples of [1_000, 5_000, 10_000, 50_000, 100_000]) {
  Deno.bench({
    name: `Delete ${samples} triples (NodeIDType format, high duplicates)`,
    group: "TribbleDB Deletion (NodeIDType)",
    fn: (bench) => {
      const ID_LENGTH = 5;
      const TYPE_LENGTH = 5;
...
for (const samples of [1_000, 5_000, 10_000, 50_000, 100_000]) {
  Deno.bench({
    name: `Delete ${samples} triples (NodeIDTypeQs format, high duplicates)`,
    group: "TribbleDB Deletion (NodeIDTypeQs)",
    fn: (bench) => {
      const ID_LENGTH = 5;
      const TYPE_LENGTH = 5;
